    ...
    describe("Mongo 'animals' collection", function () {
        ...
        before(function () {
            ...
            testInsertAnimal = function (result, callback) {
                ...
                result.state.should.equal(newAnimal.state);
                callback();
            }
        });
        it('Insert animal in DB', function (done) {
            newAnimal.save(function (err, doc) {
                if (err) {
                    ...
                    newAnimalId = doc._id;
                    newAnimal.animal_id = doc.animal_id;
                    testInsertAnimal(doc, done);
                }
            })
        });
        it('Update animal in DB', function (done) {
            const newAnimalData = { name: "TestAnimalUpdate" };
            Animal.findOneAndUpdate({ _id: newAnimalId }, newAnimalData, { new: true }, function (err, doc) {
                if (err) {
                    done(err);
                } else {
                    doc.name.should.equal(newAnimalData.name);
                    done();
                }
            });
        });
